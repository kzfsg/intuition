...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate Intuition and choose when to capture.", MAX_ATTACHMENTS);
	...
	private getSourceOptions(): { label: string; value: IssueSource }[] {
		const options: { label: string; value: IssueSource }[] = [
			{ label: product.nameLong || localize('vscode', "Intuition"), value: IssueSource.VSCode },
			{ label: localize('extensionSource', "An Intuition extension"), value: IssueSource.Extension },
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			case IssueSource.VSCode:
				return product.nameLong || localize('vscode', "Intuition");
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "An Intuition extension");
			case IssueSource.Marketplace:
